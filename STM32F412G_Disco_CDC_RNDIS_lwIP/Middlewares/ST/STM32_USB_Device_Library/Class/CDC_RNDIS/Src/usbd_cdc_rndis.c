...
USBDeviceQualifierDescriptor_t USBD_CDC_RNDIS_DeviceQualifierDesc = {
...
static uint32_t ConnSpeedTab[2] = {
		CDC_RNDIS_CONNECT_SPEED_UPSTREAM,
...
USBDeviceConfigurationDescriptor_t USBD_CDC_RNDIS_CfgFSDesc = {
		// regular configuration descriptor
		{
				sizeof(USBConfigurationDescriptor_t),
				USB_DESC_TYPE_CONFIGURATION,
				sizeof(USBDeviceConfigurationDescriptor_t),
				0x02,
				0x01,
				0x00,
				0xc0,
...
//Linux or windows 7
		...
		{
				sizeof(USBCDCHeaderDescriptor_t),
		...
		{
				sizeof(USBCDCCallManagementDescriptor_t),
				CDC_CS_INTERFACE,                         //bDescriptorType
				CDC_CALL_MANAGEMENT_DESC_SUBTYPE,         //bDescriptorSubtype
				0x00,                                     //bmCapabilities
				CDC_RNDIS_COM_ITF_NBR                                         //bDataInterface
		},
		// CDC abstract control mgmt func descr
		{
				sizeof(USBCDCACMDescriptor_t),
				CDC_CS_INTERFACE,                         //bDescriptorType
		...
		{
				sizeof(USBCDCUnionDescriptor_t),
				CDC_CS_INTERFACE,                         //bDescriptorType
				CDC_UNION_DESC_SUBTYPE,                   //bDescriptorSubtype
				CDC_RNDIS_CMD_ITF_NBR,
				CDC_RNDIS_COM_ITF_NBR,
		},
...
//USBD_CDC_RNDIS_HandleTypeDef m_hcdc;
...
static uint8_t USBD_CDC_RNDIS_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx) {
	...
	if (pdev->dev_speed == USBD_SPEED_HIGH) {
		/* Open EP IN */
		(void) USBD_LL_OpenEP(pdev, CDC_RNDIS_IN_EP, USBD_EP_TYPE_BULK, CDC_RNDIS_DATA_HS_IN_PACKET_SIZE);

		pdev->ep_in[CDC_RNDIS_IN_EP & 0xFU].is_used = 1U;

		/* Open EP OUT */
		(void) USBD_LL_OpenEP(pdev, CDC_RNDIS_OUT_EP, USBD_EP_TYPE_BULK, CDC_RNDIS_DATA_HS_OUT_PACKET_SIZE);
		...
		/* Open EP IN */
		(void) USBD_LL_OpenEP(pdev, CDC_RNDIS_IN_EP, USBD_EP_TYPE_BULK,CDC_RNDIS_DATA_FS_IN_PACKET_SIZE);

		pdev->ep_in[CDC_RNDIS_IN_EP & 0xFU].is_used = 1U;

		/* Open EP OUT */
		(void) USBD_LL_OpenEP(pdev, CDC_RNDIS_OUT_EP, USBD_EP_TYPE_BULK,CDC_RNDIS_DATA_FS_OUT_PACKET_SIZE);
	...
	(void) USBD_LL_PrepareReceive(pdev, CDC_RNDIS_OUT_EP, hcdc->RxBuffer, hcdc->MaxPcktLen);
...
static uint8_t USBD_CDC_RNDIS_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum) {
	...
	if (epnum == (CDC_RNDIS_IN_EP & 0x7FU)) {
		...
		if (hcdc->NotificationStatus != 0U) {
			(void) USBD_CDC_RNDIS_SendNotification(pdev, CONNECTION_SPEED_CHANGE, 0U, (uint8_t *) ConnSpeedTab);
...
static uint8_t USBD_CDC_RNDIS_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum) {
	...
	if (epnum == CDC_RNDIS_OUT_EP) {
		...
		if ((CurrPcktLen < hcdc->MaxPcktLen) || (hcdc->RxLength >= (CDC_RNDIS_ETH_MAX_SEGSZE + sizeof(USBD_CDC_RNDIS_PacketMsgTypeDef)))) {
			...
			/* Note: Important to RESET the RxLength. That was a bug in the initial implementation by ST */
			hcdc->RxLength = 0;

		} else {
			/* Prepare Out endpoint to receive next packet in current/new frame */
			(void) USBD_LL_PrepareReceive(pdev, CDC_RNDIS_OUT_EP, (uint8_t *) (hcdc->RxBuffer + hcdc->RxLength), hcdc->MaxPcktLen);
		}
	} else {
		return (uint8_t) USBD_FAIL;
	}
...
static uint8_t *USBD_CDC_RNDIS_GetFSCfgDesc(uint16_t *length)
{
  *length = (uint16_t)(sizeof(USBDeviceConfigurationDescriptor_t));

  return (uint8_t*)&USBD_CDC_RNDIS_CfgFSDesc;
}
...
uint8_t *USBD_CDC_RNDIS_GetDeviceQualifierDescriptor(uint16_t *length)
{
  *length = (uint16_t)(sizeof(USBDeviceQualifierDescriptor_t));

  return (uint8_t*)&USBD_CDC_RNDIS_DeviceQualifierDesc;
}
...
uint8_t USBD_CDC_RNDIS_TransmitPacket(USBD_HandleTypeDef *pdev) {
	...
	if (hcdc->TxState == 0U) {
		...
		//USBD_CDC_RNDIS_ReceivePacket(pdev);
...
static uint8_t USBD_CDC_RNDIS_ProcessPacketMsg(USBD_HandleTypeDef *pdev,  USBD_CDC_RNDIS_PacketMsgTypeDef *Msg)
{
  ...
  // avoid that the RxBuffer is moved constantly to the back...
  uint8_t* buf =  (uint8_t *)(tmp1 + tmp2 + CDC_RNDIS_PCKTMSG_DATAOFFSET_OFFSET);
  uint32_t rxLen = PacketMsg->DataLength;
  ...
  ((USBD_CDC_RNDIS_ItfTypeDef *)pdev->pUserData)->Receive(buf, &rxLen);
